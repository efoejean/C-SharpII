...
namespace StudentDelegate
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var ordered = students.OrderBy(s => s.FullName.Substring(1));

            ShowStudents("Students in order by 2 character:", ordered.ToList());
            ...
            var ftfs = students.Where(s => s.CurrentCredits >= 12 && s.TotalCredits < 12)
                .Select(s => new { s.FullName, Classes = s.CurrentCredits / 3 });

            Console.WriteLine("");
            Console.WriteLine("FT first semester students");
            Console.WriteLine("#   Student Name     Classes ");
            Console.WriteLine("--- -------------- ----------");

            foreach(var s in ftfs)
            {
                Console.WriteLine($"{s.FullName} has {s.Classes} classes");
            }
